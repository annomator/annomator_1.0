...
# Using pbtxt for congruency with pipeline.config
...
IMAGES_DIR = os.path.join('..', 'annotate', 'test_images')
...
JSON_PATH = os.path.join('..', 'annotate', 'test_images_summary.json')
...
PIXEL_AREA_MIN = 0 # Leave at zero where no data exists
...
for image in json_loaded['images']:
    image_count +=1
    image_path = os.path.join(IMAGES_DIR, image['image'])
    if os.path.exists(image_path):
        ...
        skipped_images +=1
        print("No image found for json reference", image['image'])
...
# Create tf records
train_image_count = tf_record.create_tf_record_from_json_annotate(
    json_loaded, category_index,
    SAMPLE_CATEGORIES, CATEGORY_SAMPLE_NUM, 
    valid_cat_ids, translation_index, 
    train_images, IMAGES_DIR, heights, widths, 
    TRAIN_RECORD_PATH)

if SAMPLE_IMAGES:
    val_image_count = tf_record.create_tf_record_from_json_annotate(
        json_loaded, category_index,
        SAMPLE_CATEGORIES, CATEGORY_VAL_NUM, 
        valid_cat_ids, translation_index, 
        val_images, IMAGES_DIR, heights, widths, 
        VAL_RECORD_PATH)
